...
cases |> 
  ...
  mutate(date = ymd(date),
         ...
         year = year(date)) |>
  unite(col = month_year, month, year, sep = " ", remove = TRUE, na.rm = FALSE) |>
  group_by(month_year)|>
  summarize(monthly_cases = sum(cases_daily))|>
  ggplot(aes(x = fct_reorder(month_year, my(month_year)), y = monthly_cases), 
         ) +
  geom_point() +
  coord_flip()
...
#| label: ca-2021-covid
cases |> 
  filter(state == "California", year(date) == "2021") |> 
  mutate(date = ymd(date),
         month = month(date, label = TRUE, abbr = FALSE),
         year = year(date)) |>
  unite(col = month_year, month, year, sep = " ", remove = TRUE, na.rm = FALSE) |>
...
cases |> 
  ...
  geom_boxplot() +
  labs(title = "California Covid-19 cases from January 2020 to March 2023",
       x = "Number of Cases",
       y = "Month by Year")+
  coord_flip()
